...
- 라우터가 경로 정보를 얻는 방법은 크게 3가지 방법으로 구분할 수 있다
    1. 다이렉트 커넥티드
        ...
        - 라우터나 PC에서는 이 정보를 통해 라우팅 테이블을 자동으로 만듬
...
- 말 그대로 경로를 디폴드값으로 지정하는 경우로, 경로를 찾지 못한 모든 네트워크들의 경로를 미리 지정해 놓는 것
- 주로 인터넷을 사용하는 라우터나 경로가 하나뿐인 경우 사용
```

> ### [5.3.3] 다이나믹 라우팅
- 네트워크가 몇 개 없는 간단한 네트워크 구조에서는 스태틱 라우팅으로 망을 유지하는것이 가능
- 하지만 일반적으로는 SPoF를 없애기 위해 두 개이상의 경로를 유지
    - 이 경우 대체 경로에 대한 고민이 필요
    - 만약 스태틱 라우팅으로만 네트워크를 구성하면 한 홉이 넘어간 네트워크 상태가 변경시 수동으로 대응해야 하기 때문

- 다이나믹 라우팅 프로토콜을 사용하면 관리자의 직접적인 개입 없이 라우터끼리 정보를 교환해 경로 정보를 최신으로 유지할 수 있다
- 주기적으로 특별한 변화가 있으며 경로 정보를 교환하므로 중간 경로에 문제가 발생하더라도 대체경로를 찾는 작업이 자동으로 수행한다

- 다이나믹 라우팅 프로토콜은 네트워크나 주변 기술의 변화에 맞춰 다양한 프로토콜이 존재한다
