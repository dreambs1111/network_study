...
- 네트워크 정보를 쉽게 추가하고 경로를 직접 제어할 수 있는 가장 강력한 방법은 스태틱 라우팅이다
__라우팅 설정 문법__
```
ip route NETWORK NETMAST NEXTHOP            [네트워크 장비 : 시스코]
route add -net NETWORK /Prefix gw NEXTHOP   [서버 운영 체제 : 리눅스]
```
- 장비나 운영체제에 다라 일부 문법은 다르지만 대부분 위와 같은 문법체계층을 사용한다
- 해석 : "목적지(네트워크/호스트 - 서브넷/서브넷 마스크)로 가려면 패킷을 넥스트홉으로 보내야 한다"

- 3계층 장비인 라우터는 적절한경로 정보가 없으면 패킷을 버린다
    - 회사내의 네트워크 규모정도는 스태틱 라우팅으로도 문제가 없으나 규모가 커지거나 인터넷연결이 필요시에는 어려움이 있다
    - 따라서 대용량의 인터넷 라우팅 전용 라우터가 존재하고 대부분 통신사같은 인터넷 사업자가 운영한다

- 그렇다면 회사에서 운영하는 라우터는 인터넷의 모든 라우팅 정보를 가질만큼 크지 않은데 어떻게 인터넷 경로 정보를 받아올까
    - 디폴트라우팅(스태틱라우팅의 확장)
